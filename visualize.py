...
def visualize_train_process(data, models, device):
    ...
    dense_preserve_mask = torch.FloatTensor((data['densepose'].cpu().numpy() == 15).astype(np.int64)) + torch.FloatTensor((data['densepose'].cpu().numpy() == 16).astype(np.int64)) \
                            ...
                            + torch.FloatTensor((data['densepose'].cpu().numpy() == 21).astype(np.int64)) + torch.FloatTensor((data['densepose'].cpu().numpy() == 22).astype(np.int64))
    ...
    premap = premap * 2 - 1
    # parsemap = (parsemap.squeeze() / 255)* 2 - 1
    
    denseposemap = (torch.argmax(densepose.squeeze(), dim=0) / densepose.shape[1] * 2 - 1)
    # pil_img = Image.fromarray(denseposemap)

    # denseposemap = torch.zeros((256, 192)).to(device)
    # for i in range(len(densepose.squeeze().shape)):
    #     layer = (densepose[0][i].to(device) + 1) / 2
    #     denseposemap += (layer * i / densepose.shape[1])
    # denseposemap * 2 - 1

    posemap, _ = torch.max(pose.squeeze(), 0)
    human_map = torch.cat([premap.to(device), denseposemap.to(device), posemap.to(device)], 1)

    dense_preserve =  torch.cat([dense_preserve_mask, dense_preserve_mask, dense_preserve_mask], 1) * 2 - 1
    preserve_map = torch.cat([preserve_region[0].to(device), dense_preserve[0].to(device)], 2)

    return human_map, preserve_map, real_image[0], clothes_un[0], warped_cloth_un[0], p_tryon_un[0], clothes[0], warped_cloth[0], p_tryon[0]
...
def visualize(opt):
    ...
    for data in train_loader:
        human_map, preserve_map, real_image, clothes_un, warped_cloth_un, p_tryon_un, clothes, warped_cloth, p_tryon = visualize_train_process(
            data,
            models={'pb_warp': pb_warp_model, 'pb_gen': pb_gen_model, 'pf_warp': pf_warp_model, 'pf_gen': pf_gen_model},
            device=device,
        )

        tv.utils.save_image(human_map, samples_dir / 'human_map.jpg', nrow=int(1), normalize=True, value_range=(-1,1))
        tv.utils.save_image(preserve_map, samples_dir / 'preserve_map.jpg', nrow=int(1), normalize=True, value_range=(-1,1))
        tv.utils.save_image(real_image, samples_dir / 'real_image.jpg', nrow=int(1), normalize=True, value_range=(-1,1))
        tv.utils.save_image(clothes_un, samples_dir / 'clothes_un.jpg', nrow=int(1), normalize=True, value_range=(-1,1))
        tv.utils.save_image(warped_cloth_un, samples_dir / 'warped_cloth_un.jpg', nrow=int(1), normalize=True, value_range=(-1,1))
        tv.utils.save_image(p_tryon_un, samples_dir / 'p_tryon_un.jpg', nrow=int(1), normalize=True, value_range=(-1,1))
        tv.utils.save_image(clothes, samples_dir / 'clothes.jpg', nrow=int(1), normalize=True, value_range=(-1,1))
        tv.utils.save_image(warped_cloth, samples_dir / 'warped_cloth.jpg', nrow=int(1), normalize=True, value_range=(-1,1))
        tv.utils.save_image(p_tryon, samples_dir / 'p_tryon.jpg', nrow=int(1), normalize=True, value_range=(-1,1))

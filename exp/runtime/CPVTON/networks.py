...
class FeatureRegression(nn.Module):
    def __init__(self, input_nc=512, output_dim=6, device="cpu"):
        super().__init__()
        ...
        self.linear = nn.Linear(64 * 4 * 3, output_dim).to(device)
...
class TpsGridGen(nn.Module):
    def __init__(
        self, out_h=256, out_w=192, use_regular_grid=True, grid_size=3, reg_factor=0, device="cpu"
    ):
        super().__init__()
        ...
        self.grid_X = torch.FloatTensor(self.grid_X).unsqueeze(0).unsqueeze(3).to(device)
        self.grid_Y = torch.FloatTensor(self.grid_Y).unsqueeze(0).unsqueeze(3).to(device)
        ...
        if use_regular_grid:
            ...
            self.P_X_base = P_X.clone().to(device)
            self.P_Y_base = P_Y.clone().to(device)
            self.Li = self.compute_L_inverse(P_X, P_Y).unsqueeze(0)
            self.P_X = P_X.unsqueeze(2).unsqueeze(3).unsqueeze(4).transpose(0, 4).to(device)
            self.P_Y = P_Y.unsqueeze(2).unsqueeze(3).unsqueeze(4).transpose(0, 4).to(device)
...
class GMM(nn.Module):
    ...
    def __init__(self, fine_height, fine_width, grid_size, device):
        ...
        self.regression = FeatureRegression(
            input_nc=192, output_dim=2 * grid_size**2, device=device
        )

...
from CVTON.bpgm.model.blocks import ConvBlock, Down, FeatureL2Norm
from CVTON.bpgm.model.utils import construct_basegrid, init_weights
...
class TpsGridGen(nn.Module):
    def __init__(
        self, out_h=256, out_w=192, use_regular_grid=True, grid_size=3, reg_factor=0, use_cuda=True
    ):
        super().__init__()
...
class BPGM(nn.Module):
    ...
    def __init__(self, opt):
        ...
        if "bpgm_id" in opt:
            ...
            self.resolution = (
                self.resolution,
                int(self.resolution * (opt.img_size[1] / opt.img_size[0])),
            )
        ...
        self.extractionA = FeatureExtraction(
            self.in_channels, ngf=ngf, n_layers=n_layers, norm_layer=nn.BatchNorm2d
        )

        self.extractionB = FeatureExtraction(
            3, ngf=ngf, n_layers=n_layers, norm_layer=nn.BatchNorm2d
        )
        self.l2norm = FeatureL2Norm()
        self.correlation = FeatureCorrelation()
        self.regression = FeatureRegression(
            input_nc=self.resolution[1],
            output_dim=2 * opt.grid_size**2,

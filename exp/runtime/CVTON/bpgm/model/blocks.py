...
class Down(nn.Module):
    def __init__(self, *args, pooling=nn.MaxPool2d, **kwargs):
        ...
        self.blocks = nn.Sequential(pooling(2), ConvBlock(*args, **kwargs))
...
class ConvBlock(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.Sequential(
            spectral_norm(
                conv(in_channel, mid_channel, kernel_size=kernel_size[0], padding=padding[0])
            ),
            normalization(out_channel),
            nn.LeakyReLU(relu_slope),
            spectral_norm(
                conv(mid_channel, out_channel, kernel_size=kernel_size[1], padding=padding[1])
            ),
            normalization(out_channel),
            nn.LeakyReLU(relu_slope),
        )

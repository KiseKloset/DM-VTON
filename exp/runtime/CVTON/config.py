...
def get_test_arguments():
    ...
    opt.label_nc = [
        len(semantic_body_labels),
        len(semantic_cloth_labels),
        len(semantic_densepose_labels),
    ]
...
def add_all_arguments(parser, train):
    ...
    parser.add_argument(
        '--which_iter', type=str, default='latest', help='which epoch to load when continue_train'
    )
    parser.add_argument(
        '--gpu_ids', nargs='+', default=[0], type=int, help="GPUs to use for training / inference"
    )

    # for generator
    parser.add_argument(
        '--num_res_blocks', type=int, default=6, help='number of residual blocks in G and D'
    )
    parser.add_argument(
        '--channels_G',
        type=int,
    ...
    parser.add_argument(
        '--EMA_decay', type=float, default=0.9999, help='decay in exponential moving averages'
    )
    parser.add_argument('--z_dim', type=int, default=9, help="dimension of the latent z vector")

    parser.add_argument(
        '--val_size', type=float, default=0.05, help="Validation set size (fraction - not int)."
    )
    parser.add_argument(
        '--train_size', type=float, default=0.95, help="Train set size (fraction - not int)."
    )
    parser.add_argument(
        '--transform_cloth',
        action='store_true',
    ...
    if train:
        parser.add_argument(
            '--freq_print', type=int, default=1000, help='frequency of showing training results'
        )
        parser.add_argument(
            '--freq_save_ckpt', type=int, default=10000, help='frequency of saving the checkpoints'
        )
        parser.add_argument(
            '--freq_save_latest',
            type=int,
        ...
        parser.add_argument(
            ...
            type=int,
            default=250,
            help='smoothing window for loss visualization',
        )
        parser.add_argument(
            '--freq_save_loss', type=int, default=2000, help='frequency of loss plot updates'
        )
        parser.add_argument(
            '--freq_fid',
            type=int,
        ...
        parser.add_argument(
            '--continue_train', action='store_true', help='resume previously interrupted training'
        )
        parser.add_argument('--num_epochs', type=int, default=200, help='number of epochs to train')
        parser.add_argument('--beta1', type=float, default=0.0, help='momentum term of adam')
        parser.add_argument('--beta2', type=float, default=0.999, help='momentum term of adam')
        parser.add_argument(
            '--lr_g', type=float, default=0.0001, help='G learning rate, default=0.0001'
        )
        parser.add_argument(
            '--lr_d', type=float, default=0.0004, help='D learning rate, default=0.0004'
        )
        ...
        parser.add_argument(
            '--add_pd_loss', action="store_true", help="if specified, add patch discriminator loss"
        )
        parser.add_argument(
            '--patch_size', type=int, default=0, help="patch size for patch discriminator"
        )
        parser.add_argument(
            '--no_balancing_inloss',
            action='store_true',
        ...
        parser.add_argument(
            '--lambda_labelmix', type=float, default=10.0, help='weight for LabelMix regularization'
        )

    else:
        parser.add_argument(
            '--results_dir', type=str, default='./results/', help='saves testing results here.'
        )

...
import CVTON.models.norms as norms
import numpy as np
import torch
import torch.nn as nn
import torch.nn.functional as F
from CVTON.bpgm.model.models import BPGM
from CVTON.models.blocks import Encoder
...
class Simple_Generator(nn.Module):
    def __init__(self, opt):
        ...
        semantic_nc = np.sum(
            [
                nc
                for mode, nc in zip(["body", "cloth", "densepose"], opt.semantic_nc)
        ...
        for i in range(len(self.channels) - 1):
            self.body.append(
                ResnetBlock(self.channels[i] + opt.z_dim + semantic_nc, self.channels[i + 1], opt)
            )
    ...
    def forward(self, seg, z=None):
        ...
        for i in range(self.opt.num_res_blocks - 1, -1, -1):
            ...
            _seg = F.interpolate(
                seg, scale_factor=scale, mode="nearest", recompute_scale_factor=False
            )
            _z = F.interpolate(z, scale_factor=scale, recompute_scale_factor=False)
    ...
    def forward(self, I_m, C_t, body_seg, cloth_seg, densepose_seg, agnostic=None):
        ...
        if len(self.opt.segmentation) == 1:
            ...
            seg = torch.cat(
                [
                    seg_dict[mode]
                    for mode in sorted(seg_dict.keys())
                    if mode in self.opt.segmentation
                ],
                axis=1,
            )
        ...
        if self.bpgm is not None:
            with torch.no_grad():
                ...
                if self.bpgm.resolution != self.opt.img_size:
                    ...
                    _C_t = F.interpolate(
                        C_t, size=self.bpgm.resolution, mode="bilinear", align_corners=False
                    )
                    grid = self.bpgm(_seg, _C_t).permute(0, 3, 1, 2)

                    grid = F.interpolate(
                        grid, size=self.opt.img_size, mode="bilinear", align_corners=False
                    )

...
def DAWarp(feat, offsets, att_maps, sample_k, out_ch, detach=False):
    ...
    if detach:
        multi_warp_feat = F.grid_sample(
            multi_feat, offsets.detach().permute(0, 2, 3, 1), mode='bilinear', padding_mode='border'
        )
    else:
        multi_warp_feat = F.grid_sample(
            multi_feat, offsets.permute(0, 2, 3, 1), mode='bilinear', padding_mode='border'
        )
        ...
        for i in range(len(source_feats)):
            ...
            if last_multi_cross_offsets is not None:
                offsets = F.grid_sample(
                    last_multi_cross_offsets, offsets, mode='bilinear', padding_mode='border'
                )
            else:
                offsets = offsets.permute(0, 3, 1, 2)
            last_multi_cross_offsets = offsets
            att_source_feat = DAWarp(
                feat_source, last_multi_cross_offsets, cross_att_maps, self.k, self.out_ch
            )
            ...
            if last_multi_self_offsets is not None:
                offsets = F.grid_sample(
                    last_multi_self_offsets, offsets, mode='bilinear', padding_mode='border'
                )
            else:
                offsets = offsets.permute(0, 3, 1, 2)
            ...
            last_multi_cross_offsets = F.grid_sample(
                last_multi_cross_offsets, cross_offsets, mode='bilinear', padding_mode='border'
            )
            last_multi_self_offsets = F.grid_sample(
                last_multi_self_offsets, self_offsets, mode='bilinear', padding_mode='border'
            )
            ...
            if return_all:
                ...
                if use_light_en_de:
                    ...
                    warp_att_source_image = DAWarp(
                        cur_source_image, last_multi_cross_offsets, cross_att_maps, self.k, 64
                    )
                    warp_att_reference_image = DAWarp(
                        cur_reference_image, last_multi_self_offsets, self_att_maps, self.k, 64
                    )
                    ...
                    warp_att_source_image = DAWarp(
                        cur_source_image, last_multi_cross_offsets, cross_att_maps, self.k, 3
                    )
                    warp_att_reference_image = DAWarp(
                        cur_reference_image, last_multi_self_offsets, self_att_maps, self.k, 3
                    )

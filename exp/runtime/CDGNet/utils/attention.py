...
from torch.nn import AdaptiveAvgPool2d, AdaptiveMaxPool2d, AvgPool2d
from torch.nn import BatchNorm1d as BatchNorm1d
from torch.nn import BatchNorm2d as BatchNorm2d
from torch.nn import (BCELoss, Conv2d, CrossEntropyLoss, Dropout, Embedding, Linear, MaxPool2d,
                      Module, NLLLoss, Parameter, ReLU, Sequential, Sigmoid, Softmax,)
...
class CDGAttention(nn.Module):
    def __init__(
        self, feat_in=512, feat_out=256, num_classes=20, size=[384 // 16, 384 // 16], kernel_size=7
    ):
        super().__init__()
        h, w = size[0], size[1]
        kSize = kernel_size

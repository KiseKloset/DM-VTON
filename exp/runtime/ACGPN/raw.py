...
class ACGPN(nn.Module):
    def __init__(self, checkpoint=None):
        ...
        if checkpoint is not None:
            ...
            opt.U_path = checkpoint.get('U_path', '')
            opt.G1_path = checkpoint.get('G1_path', '')
            opt.G2_path = checkpoint.get('G2_path', '')
            opt.G_path = checkpoint.get('G_path', '')
        else:
    ...
    def forward(self, person, cloth, cloth_edge, pose, parse):
        ...
        ############## Forward Pass ######################
        fake_image, warped_cloth, refined_cloth = self.model(
            Variable(parse),
            Variable(cloth_edge),
            Variable(img_fore),
        ...
        return fake_image, warped_cloth, refined_cloth

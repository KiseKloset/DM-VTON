...
def get_img_agnostic_onlypose(img, pose_data):
    ...
    for i in [2, 5]:
        ...
        agnostic_draw.ellipse(
            (pointx - r * 3, pointy - r * 3, pointx + r * 3, pointy + r * 3), 'black', 'black'
        )
    ...
    for i in [8, 11]:
        ...
        if pointx < 1 and pointy < 1:
            continue
        agnostic_draw.ellipse(
            (pointx - r * 3, pointy - r * 6, pointx + r * 3, pointy + r * 6), 'black', 'black'
        )
    line_points = []
    for i in [2, 8]:
    ...
    agnostic_draw.rectangle(
        (pointx - r * 2, pointy - r * 2, pointx + r * 2, pointy + r * 2), 'black', 'black'
    )
...
def get_img_agnostic(img, parse, pose_data):
    ...
    parse_hand = (parse_array == 14).astype(np.float32) + (parse_array == 15).astype(np.float32)
    ...
    for i in [2, 5]:
        pointx, pointy = pose_data[i]
        agnostic_draw.ellipse(
            (pointx - r * 4, pointy - r * 4, pointx + r * 5, pointy + r * 4), 'black', 'black'
        )
...
def process(img_name, save_dirname="img_agnostic"):
    ...
    with open(osp.join(data_path, 'pose', pose_name)) as f:

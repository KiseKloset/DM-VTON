...
class Pix2PixHDModel(BaseModel):
    ...
    def encode_input(self, label_map, clothes_mask, all_clothes_label):
        ...
        input_label = input_label.scatter_(
            1, label_map.data.long().to(self.device), 1.0)
        ...
        c_label = c_label.scatter_(
            1, all_clothes_label.data.long().to(self.device), 1.0)
    ...
    def forward(self, label, pre_clothes_mask, img_fore, clothes_mask, clothes, all_clothes_label, real_image, pose, grid, mask_fore):
        ...
        arm1_mask = torch.FloatTensor(
            (label.cpu().numpy() == 11).astype(np.float)).to(self.device)
        arm2_mask = torch.FloatTensor(
            (label.cpu().numpy() == 13).astype(np.float)).to(self.device)
        pre_clothes_mask = torch.FloatTensor(
            (pre_clothes_mask.detach().cpu().numpy() > 0.5).astype(np.float)).to(self.device)
        ...
        fake_cl_dis = torch.FloatTensor(
            (fake_cl.detach().cpu().numpy() > 0.5).astype(np.float)).to(self.device)
        fake_cl_dis = morpho(fake_cl_dis, 1, True, device=self.device)

        new_arm1_mask = torch.FloatTensor(
            (armlabel_map.cpu().numpy() == 11).astype(np.float)).to(self.device)
        new_arm2_mask = torch.FloatTensor(
            (armlabel_map.cpu().numpy() == 13).astype(np.float)).to(self.device)

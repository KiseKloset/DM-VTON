...
def generate_discrete_label(inputs, label_nc, onehot=True, encode=True, device='cpu'):
    pred_batch = []
    size = inputs.size()
    for input in inputs:
    ...
    input_label = torch.cuda.FloatTensor(torch.Size(oneHot_size), device=device).zero_()
    input_label = input_label.scatter_(1, label_map.data.long().to(device), 1.0)

    return input_label


def morpho(mask, iter, bigger=True, device='cpu'):
...
def morpho_smaller(mask, iter, bigger=True, device='cpu'):
...
def encode(label_map, size, device='cpu'):
    label_nc = 14
    oneHot_size = (size[0], label_nc, size[2], size[3])
    input_label = torch.cuda.FloatTensor(torch.Size(oneHot_size), device=device).zero_()
    input_label = input_label.scatter_(1, label_map.data.long().to(device), 1.0)
...
class Pix2PixHDModel(BaseModel):
    ...
    def ger_average_color(self, mask, arms):
        color = torch.zeros(arms.shape).to(self.device)
    ...
    def initialize(self, opt, device):
        ...
        # Main Generator
        with torch.no_grad():
            self.Unet = networks.define_UnetMask(4, device=self.device).eval()
            self.G1 = networks.define_Refine(37, 14, device=self.device).eval()
            self.G2 = networks.define_Refine(19 + 18, 1, device=self.device).eval()
            self.G = networks.define_Refine(24, 3, device=self.device).eval()
        ...
        if self.isTrain:
            ...
            self.criterionStyle = networks.StyleLoss(self.device)
    ...
    def encode_input(self, label_map, clothes_mask, all_clothes_label):
        ...
        input_label = torch.cuda.FloatTensor(torch.Size(oneHot_size), device=self.device).zero_()
        input_label = input_label.scatter_(1, label_map.data.long().to(self.device), 1.0)

        masked_label = torch.cuda.FloatTensor(torch.Size(oneHot_size), device=self.device).zero_()
        masked_label = masked_label.scatter_(
            1, (label_map * (1 - clothes_mask)).data.long().to(self.device), 1.0
        )

        c_label = torch.cuda.FloatTensor(torch.Size(oneHot_size), device=self.device).zero_()
        c_label = c_label.scatter_(1, all_clothes_label.data.long().to(self.device), 1.0)
    ...
    def encode_input_test(self, label_map, label_map_ref, real_image_ref, infer=False):
        if self.opt.label_nc == 0:
            ...
            input_label_ref = label_map_ref.data.to(self.device)
        else:
            ...
            input_label_ref = input_label_ref.scatter_(
                1, label_map_ref.data.long().to(self.device), 1.0
            )
        ...
        real_image_ref = Variable(real_image_ref.data.to(self.device))
    ...
    def multi_scale_blend(self, fake_img, fake_c, mask, number=4):
        ...
        for i in range(1, number + 1):
            bigger = smaller
            smaller = morpho(smaller, 2, False, device=self.device)
            mid = bigger - smaller
    ...
    def forward(
        ...
        arm1_mask = torch.FloatTensor((label.cpu().numpy() == 11).astype(np.float)).to(self.device)
        arm2_mask = torch.FloatTensor((label.cpu().numpy() == 13).astype(np.float)).to(self.device)
        ...
        G1_in = torch.cat(
            [pre_clothes_mask, clothes, all_clothes_label, pose, self.gen_noise(shape)], dim=1
        )
        arm_label = self.G1.refine(G1_in)

        arm_label = self.sigmoid(arm_label)
        CE_loss = (
            self.cross_entropy2d(arm_label, (label * (1 - clothes_mask)).transpose(0, 1)[0].long())
            * 10
        )

        armlabel_map = generate_discrete_label(arm_label.detach(), 14, False, device=self.device)
        dis_label = generate_discrete_label(arm_label.detach(), 14, device=self.device)
        ...
        fake_cl_dis = torch.FloatTensor((fake_cl.detach().cpu().numpy() > 0.5).astype(np.float)).to(
            self.device
        )
        fake_cl_dis = morpho(fake_cl_dis, 1, True, device=self.device)

        new_arm1_mask = torch.FloatTensor((armlabel_map.cpu().numpy() == 11).astype(np.float)).to(
            self.device
        )
        new_arm2_mask = torch.FloatTensor((armlabel_map.cpu().numpy() == 13).astype(np.float)).to(
            self.device
        )
        ...
        bigger_arm1_occ = morpho(arm1_occ, 10, device=self.device)
        bigger_arm2_occ = morpho(arm2_occ, 10, device=self.device)
        ...
        dis_label = encode(armlabel_map, armlabel_map.shape, device=self.device)
    ...
    def update_learning_rate(self):
        ...
        for param_group in self.optimizer_G.param_groups:
            param_group['lr'] = lr
        if self.opt.verbose:
            print('update learning rate: {:f} -> {:f}'.format(self.old_lr, lr))
        self.old_lr = lr

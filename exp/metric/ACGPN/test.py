...
def run_test(dataroot, save_dir, batch_size, device):
    ...
    for i, data in enumerate(dataset):
        ...
        fake_image, warped_cloth, refined_cloth = model(
            Variable(data['label'].to(device)),
            Variable(data['edge'].to(device)),
            Variable(img_fore.to(device)),
            Variable(mask_clothes.to(device)),
            Variable(data['color'].to(device)),
            Variable(all_clothes_label.to(device)),
            Variable(data['image'].to(device)),
            Variable(data['pose'].to(device)),
            Variable(data['image'].to(device)),
            Variable(mask_fore.to(device)),
        )

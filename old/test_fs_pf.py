...
from utils.utils import Profile, flag


def run_test(opt, device):
    test_data = LoadVITONDataset(path=opt.dataroot, phase='test', size=(256, 192))

    dataloader = DataLoader(test_data, batch_size=opt.batchSize, shuffle=False, num_workers=16)

    warp_model = AFWM(3, opt.align_corners)
    warp_model.eval()
    warp_model.to(device)
    smart_pretrained(warp_model, opt.warp_checkpoint)

    gen_model = MobileNetV2_unet(7, 4)
    gen_model.eval()
    gen_model.to(device)
    smart_pretrained(gen_model, opt.gen_checkpoint)
...
if __name__ == "__main__":
    ...
    # device = select_device(opt.gpu_ids[0], batch_size=opt.batchSize)

...
val_data = CreateDataset(opt, phase='val') 
val_sampler = DistributedSampler(train_data)
val_loader = DataLoader(train_data, batch_size=opt.batchSize, shuffle=False,
...
for epoch in range(start_epoch, opt.niter + opt.niter_decay + 1):
    ...
    if val_loss < best_val_loss:
        ...
        save_checkpoint(model.module, os.path.join(opt.checkpoints_dir, opt.name, 'PBAFN_warp_best.pth'))
        print('Saving best at epoch %d' % epoch)  
    #######################

    # end of epoch 
    iter_end_time = time.time()
    if opt.local_rank == 0:
      print('End of epoch %d / %d: train_loss: %.3f \t val_loss: %.3f \t time: %d sec' %
            (epoch, opt.niter + opt.niter_decay, train_loss, val_loss, time.time() - epoch_start_time))

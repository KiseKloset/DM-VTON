...
def train_batch(data, models, optimizers, criterions, writer, global_step, samples_dir):
    ...
    # Visualize
    if global_step % opt.sample_step == 0:
...
def train_pb_e2e(opt, device):
    ...
    warp_ckpt = smart_pretrained(warp_model, opt.pb_warp_checkpoint, name='parser-based warp')
    gen_model = ResUnetGenerator(8, 4, 5, ngf=64, norm_layer=nn.BatchNorm2d)
    gen_model.to(device)
    gen_ckpt = smart_pretrained(gen_model, opt.pb_gen_checkpoint, name='parser-based gen')

    # Optimizer
    warp_optimizer = smart_optimizer(model=warp_model, name=opt.optimizer, lr=0.2*opt.lr, momentum=opt.momentum)
    gen_optimizer = smart_optimizer(model=gen_model, name=opt.optimizer, lr=opt.lr, momentum=opt.momentum)
    ...
    if opt.resume:
        ...
        if gen_ckpt: # resume with information of gen_model
            start_epoch = smart_resume(gen_ckpt, gen_optimizer, opt.pb_gen_checkpoint, epoch_num=epoch_num)
    else:
        start_epoch = 1
    ...
    for epoch in range(start_epoch, epoch_num + 1):
        ...
        torch.save(warp_ckpt, weights_dir / f'pb_warp_last.pt')
        torch.save(gen_ckpt, weights_dir / f'pb_gen_last.pt')
        if epoch % opt.save_period == 0:      
            torch.save(warp_ckpt, weights_dir / f'pb_warp_epoch_{epoch}.pt')
            torch.save(gen_ckpt, weights_dir / f'pb_gen_epoch_{epoch}.pt')

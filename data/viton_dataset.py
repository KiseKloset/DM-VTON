...
class LoadVITONDataset(Dataset):
    def __init__(
        ...
        self.transform_parse = get_transform(train=(self.phase=='train'), method=Image.NEAREST, normalize=False)

        self.img_names, self.cloth_names = [], []

        with open(Path(self.dataroot) / f"{phase}_pairs.txt", 'r') as f:
    ...
    def __getitem__(self, index: int) -> dict:   
        ...
        if self.phase=='train':
            ...
            un_cloth = Image.open(Path(self.dataroot) / f'{self.phase}_color' / un_c_name).convert('RGB')
            # un_cloth = un_cloth.resize((self.width, self.height))
            un_cloth_tensor = self.transform_image(un_cloth)   # [-1,1]

            # Unpaired Clothing edge 
            un_cloth_edge = Image.open(Path(self.dataroot) / f'{self.phase}_edge' / un_c_name).convert('L')
            ...
            with open(Path(self.dataroot) / f'{self.phase}_pose' / f'{Path(im_name).stem}.json', 'r') as f:
            ...
            # Densepose
            dense_mask = np.load(Path(self.dataroot) / f'{self.phase}_densepose' / f'{Path(im_name).stem}.npy').astype(np.float32)

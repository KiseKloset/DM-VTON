...
def train_batch(data, models, optimizers, criterions, device, writer, global_step, sample_step, samples_dir):
    ...
    for num in range(5):
        ...
        # if num==4 and (global_step % 300 == 0):
        #     print_log(log_path, f'loss: {str(loss_l1)}, {str(loss_vgg)}, {str(loss_edge)}, {str(loss_flow_x)}, {str(loss_flow_y)}, {str(loss_all)}')
...
def train_pb_warp(opt):
    ...
    if opt.resume:
        if warp_ckpt:
            start_epoch = smart_resume(warp_ckpt, warp_optimizer, opt.pb_warp_checkpoint, epoch_num=epoch_num)
    else:
        start_epoch = 1
    ...
    for epoch in range(start_epoch, epoch_num + 1):
        ...
        if epoch % opt.save_period == 0:
            ...
            print_log(log_path, 'Saving the model at the end of epoch %d, iters %d' % (epoch, total_steps))        
        del warp_ckpt
